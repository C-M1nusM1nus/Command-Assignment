...
{
    /**
     * Creates a remote for managing a tv and lights.
     * Here the client connects the receivers (tv and lights) to concrete commands (ie Power) and sets them in the invoker (Remote)
     * 
     * @param tv the tv to connect the remote to
     * @param lights the lights to connect the remote to
    ...
    public static Remote tvRemote(Television tv, Scanner userInput)
    {
        ...
        tvRemote.setButton(new AddLightCommand(lights, "Living Room"), 8);
        tvRemote.setButton(new RemoveLightCommand(lights, "Living Room"), 9);
        return tvRemote;
    }
}
